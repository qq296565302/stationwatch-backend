...
| `mysql` | MySQL，内存缓存 + 异步落库（`MYSQL_*` 配置） | **生产部署（当前）** |

> MySQL 模式：启动自动建库建表（10 张，含 districts/stations/users/records/items/dictionary 等）、空库自动写入种子数据；flush 用单事务「DELETE 全量 + 分批 INSERT」幂等同步，失败整体回滚重试。
...
- 首次启动：自动写入种子数据（8 区县 / 1 站点 / 17 用户 / 字典 / 系统配置，含三级组织演示账号）
...
| lidong 等 15 名值班员 | @zbdl-95598 | 值班员 | 1 (马尚供电所) |
